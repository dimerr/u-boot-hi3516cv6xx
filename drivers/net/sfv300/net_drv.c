...
static struct bspeth_netdev_local bspeth_devs_priv[MAX_PORT] = {
	{
		.port = UP_PORT,
...
static int eth_set_host_mac_address(struct eth_device* const dev)
{
	...
	if (memset_s(mac, sizeof(mac), 0, sizeof(mac)))
		printf("memset_s failed!\n");
	if (eth_env_get_enetaddr("ethaddr", mac) == 0) {
...
static void phy_print_status(struct bspeth_netdev_local const *ld, unsigned int stat)
{
	printf("%s : phy status change : LINK=%s : DUPLEX=%s : SPEED=%s\n",
		   ...
		   ((stat & ETH_LINKED) != 0) ? "UP" : "DOWN",
		   ((stat & ETH_DUP_FULL) != 0) ? "FULL" : "HALF",
		   ((stat & ETH_SPD_100M) != 0) ? "100M" : "10M");
}
...
retry:
	...
	stat |= (miiphy_link(ld->mii_name, ld->phy_addr) != 0) ? ETH_LINKED : 0;
	...
	if ((--timeout_us != 0) && ((stat & ETH_LINKED) == 0))
		goto retry;
...
static void bspeth_get_phy_intf(struct bspeth_netdev_local *ld)
{
	...
	if (mdio_intf != NULL) {
		if (strncmp(mdio_intf, "mii", strlen("mii")) == 0) {
			ld->phy_intf = INTERFACE_MODE_MII;
		} else if (strncmp(mdio_intf, "rmii", strlen("rmii")) == 0) {
...
static int bspeth_init(struct eth_device *dev, struct bd_info *bd)
{
...
static int bspeth_recv(struct eth_device *dev)
{
	...
	/* receive packed, loop in NetLoop */
	while ((--timeout_us != 0) && (is_recv_packet(ld) == 0))
		udelay(1);
...
static int bspeth_send(struct eth_device *dev, void *packet, int length)
{
	...
	} while ((--timeout_us != 0) && ((ints & ud_bit_name(ETH_INT_TXQUE_RDY)) == 0));
