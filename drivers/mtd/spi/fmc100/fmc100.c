...
int write_bp_area_check(struct fmc_host* const host, u_int to, size_t len)
{
	...
	if (host->level) {
		if ((host->cmp == BP_CMP_TOP) && ((to + len) > host->start_addr)) {
			puts("\n");
			db_msg("Reg write area[%#x => %zd] was locked\n",
...
#ifdef CONFIG_DTR_MODE_SUPPORT
	unsigned int regval = fmc_read(host, FMC_GLOBAL_CFG);
	if ((regval & (1 << DTR_MODE_REQUEST_SHIFT)) == DISABLE) {
		if (memcpy_s((void *)CONFIG_SYS_TEXT_BASE_ORI, DTR_TRAINING_CMP_LEN,
...
int spi_dtr_dummy_training_set(struct fmc_host *host, int dtr_en)
{
	...
	if ((regval & (1 << DTR_MODE_REQUEST_SHIFT)) != DISABLE) {
		...
		fmc_pr(DTR_DB, " * dtr already enable,skip training.\n");
	}
...
/* In fmc100_spi_nor_init, copy the data starting from address 0 in the SPI NOR flash
 * to CONFIG_SYS_TEXT_BASE_ORI. The length of the copied data is DTR_TRAINING_CMP_LEN.
 * Used for data comparison during DTR TRAINING.
 */
					...
					unsigned char *buf)
{
	...
	fmc100_dma_transfer(spi, 0x0, buf, RW_OP_READ, DTR_TRAINING_CMP_LEN);
	...
	ret = memcmp((const void *)buf,
			(const void *)CONFIG_SYS_TEXT_BASE_ORI,
