...
static void ena_eventbuf_intr(usb3_device_t *dev)
{
...
unsigned int get_eventbuf_count(usb3_device_t *dev)
{
	unsigned int cnt;
...
static void update_eventbuf_count(usb3_device_t *dev, unsigned int cnt)
{
	usb3_wr32(&dev->core_global_regs->geventbuf[0].geventcnt, cnt);
}
...
void usb3_handle_event(usb3_device_t *dev)
{
	...
	unsigned int event, physep, intr, count, i;
	intr = 0;
