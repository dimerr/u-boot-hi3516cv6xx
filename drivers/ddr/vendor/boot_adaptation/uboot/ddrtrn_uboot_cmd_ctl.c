	...
	unsigned int win_max, unsigned int win_sum)
{
	...
	for (j = 0; j < ddrtr_data->byte_num; j++) {
		unsigned int dqs = (ddrtrn_reg_read(ddrtr_data->base_phy +
	...
	unsigned int win_max, unsigned int win_sum)
{
	...
	for (j = 0; j < ddrtr_data->byte_num; j++) {
		unsigned int dqs = ddrtrn_reg_read(ddrtr_data->base_phy + ddrtrn_hal_phy_dxnrdqsdly(j)) &
			PHY_RDQS_BDL_MASK;
		for (i = 0; i < DDR_PHY_BIT_NUM; i++) {
			dq_num = (j << 3) + i; /* shift left 3: 8 bit */
			win =
	...
	unsigned int max, unsigned int sum)
{
	...
	for (i = 0; i < DDR_PHY_CA_MAX; i++) {
		unsigned int left = ddrtr_data->ca_addr[i] >> DDR_DATAEYE_RESULT_BIT;
		unsigned int right = ddrtr_data->ca_addr[i] & DDR_DATAEYE_RESULT_MASK;
	...
	unsigned int phy_index)
{
	...
	for (i = 0; i < rank_num; i++) {
		unsigned int item = ddrtr_result->phy[phy_index].rank[i].item;
		if ((item & mask) == 0)
			enable = 1;
	}
