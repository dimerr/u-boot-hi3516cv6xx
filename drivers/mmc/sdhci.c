...
static int sdhci_transfer_data(struct sdhci_host *host, struct mmc_data *data)
{
	...
	do {
		...
		if (stat & SDHCI_INT_ERROR) {
			...
			sdhci_nebula_dumpregs(host);
#elif (defined(CONFIG_ARCH_BSP) && defined(CONFIG_BSP_SDHCI))
			if (!host->is_tuning)
				sdhci_dumpregs(host);
#endif
			return -EIO;
...
#ifdef CONFIG_MMC_SDHCI_NEBULA
		sdhci_nebula_config_mode(host, mode);
#else
...
#if (defined(CONFIG_ARCH_BSP) && !defined(CONFIG_MMC_SDHCI_NEBULA))
static void sdhci_host_power_cycle(struct mmc *mmc)
{
	u8 ctrl;
	struct sdhci_host *host = mmc->priv;
...
#if CONFIG_IS_ENABLED(MMC_HS400_ES_SUPPORT)
#ifdef CONFIG_MMC_SDHCI_NEBULA
	.set_enhanced_strobe = sdhci_enhanced_strobe,
#elif (defined(CONFIG_ARCH_BSP) && defined(CONFIG_BSP_SDHCI))
	.hs400_enable_es = sdhci_hs400_enhanced_stobe,
#endif
#endif /* MMC_HS400_ES_SUPPORT */
