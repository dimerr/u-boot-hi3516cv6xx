...
static td_s32 vo_check_enable_user_div(ot_vo_dev dev)
{
    ...
    dev_div = dev_ctx->config_user_sync_info.dev_div;
    pre_div = dev_ctx->config_user_sync_info.pre_div;
...
static td_void vo_enable_dev_clk_sel(ot_vo_dev dev)
{
    ...
    vo_drv_set_dev_clk_sel(dev, 0x8); /* 8:bpll */
}
...
{
    td_s32 ret;
    const ot_vo_manual_user_sync_info *user_sync_info = &sync_info->manual_user_sync_info;

    ret = vo_check_dev_user_div(dev, user_sync_info->dev_div, user_sync_info->pre_div);
    if (ret != TD_SUCCESS) {
        return ret;
    }
...
static vo_fixed_clk_info g_fixed_clk_info[] = {
    { OT_VO_FIXED_CLK_297M,     297000 },
    { OT_VO_FIXED_CLK_148_5M,   148500 },
    { OT_VO_FIXED_CLK_135M,     135000 },
    { OT_VO_FIXED_CLK_108M,     108000 },
    { OT_VO_FIXED_CLK_88M,       88000 },
    { OT_VO_FIXED_CLK_74_25M,    74250 },
    { OT_VO_FIXED_CLK_66M,       66000 }
};

static td_u32 vo_cal_user_clk_user_sync(const ot_vo_manual_user_sync_info *config_sync_info)
{
    td_u32 src_clk_khz = 0;
    td_u32 pixel_clk_khz = 0;
    td_u32 index = 0;
    ot_vo_pll vo_user_pll = {0};
    td_u32 size = sizeof(g_fixed_clk_info) / sizeof(vo_fixed_clk_info);

    if (config_sync_info->user_sync_attr.clk_src == OT_VO_CLK_SRC_FIXED) {
        for (index = 0; index < size; index++) {
            if (config_sync_info->user_sync_attr.fixed_clk == g_fixed_clk_info[index].fixed_clk) {
                src_clk_khz = g_fixed_clk_info[index].src_clk_khz;
                break;
            }
        }
    } else if (config_sync_info->user_sync_attr.clk_src == OT_VO_CLK_SRC_LCDMCLK) {
        src_clk_khz =
            (td_u32)(((td_u64)config_sync_info->user_sync_attr.lcd_m_clk_div * VO_LCDMCLK_BASE_CLK * VO_KHZ_CLK_UNIT) >>
            VO_LCDMCLK_SHIFT);
    } else {
        (td_void)
            memcpy_s(&vo_user_pll, sizeof(ot_vo_pll), &(config_sync_info->user_sync_attr.vo_pll), sizeof(ot_vo_pll));
        src_clk_khz = (vo_user_pll.fb_div * VO_KHZ_CLK_UNIT + ((vo_user_pll.frac * VO_KHZ_CLK_UNIT) >> VO_FRAC_SHIFT)) *
            VO_PLL_FREF / (vo_user_pll.post_div1 * vo_user_pll.post_div2);
    }

    pixel_clk_khz = src_clk_khz / config_sync_info->dev_div / config_sync_info->pre_div;
...
static td_s32 vo_do_set_user_sync_info(ot_vo_dev dev, const ot_vo_user_sync_info *sync_info)
{
    ...
    pixel_clk_khz = vo_cal_user_clk_user_sync(manual_user_sync_info);
    vo_drv_set_all_crg_clk(TD_TRUE);
    ...
    vo_drv_set_dev_user_intf_sync_attr(dev, manual_user_sync_info);

    vo_drv_set_dev_div(dev, manual_user_sync_info->dev_div);
#ifdef CONFIG_OT_HDMI_SUPPORT
    vo_drv_set_hdmi_div(dev, manual_user_sync_info->pre_div);
#endif

    vo_drv_set_clk_reverse(dev, sync_info->clk_reverse_en);

    (td_void)memcpy_s(&dev_ctx->config_user_sync_info, sizeof(ot_vo_manual_user_sync_info), manual_user_sync_info,
        sizeof(ot_vo_manual_user_sync_info));
