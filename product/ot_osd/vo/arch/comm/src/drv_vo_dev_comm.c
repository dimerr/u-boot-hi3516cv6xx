...
static td_s32 vo_drv_check_dev_pll_foutvco(ot_vo_dev dev, const ot_vo_manual_user_sync_info *sync_info)
{
    td_u32 foutvco;
    td_u32 src_clk;
    td_u32 pixel_clk;
    td_u32 max_clk;
    const td_u32 pll_fout4_divisor = 8;
    ...
    src_clk = foutvco * VO_KHZ_CLK_UNIT / (pll->post_div1 * pll->post_div2);
...
td_void vo_drv_set_rgb_param(ot_vo_dev dev, const ot_vo_rgb_param *rgb_param)
{
    ...
    ot_vo_intf_type intf = vo_drv_rgb_get_csc_intf();

    vo_hal_intf_set_csc_enable(intf, TD_TRUE);
    vou_drv_intf_csc_config(intf, &rgb_param->csc);
    ...
    vo_hal_set_intf_rgb_dummy_out(rgb_param->rgb_dummy_out_en);
    vo_hal_set_intf_rgb_dummy_order(rgb_param->rgb_dummy_order);

#ifdef CONFIG_OT_VO_INTF_GAMMA
    vo_drv_set_gamma_comm(&(rgb_param->gamma_info));
#endif
...
#ifdef CONFIG_OT_VO_INTF_GAMMA
    vo_drv_set_gamma_comm(&(mipi_param->gamma_info));
#endif
    hal_disp_set_reg_up(dev);
}
#endif
...
td_s32 vo_drv_check_intf_share(ot_vo_dev dev, ot_vo_intf_type intf_type)
{
    ...
    for (other_dev = VO_DEV_DHD0; other_dev < VO_DEV_BUTT; other_dev++) {
        if (other_dev == dev) {
            continue;
        }
