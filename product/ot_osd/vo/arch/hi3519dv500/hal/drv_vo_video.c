    ...
    const vo_video_layer_ip_cfg *vid_cfg)
{
    ...
    if ((drv_layer_ctx->vo_video_attr.pixel_format == OT_PIXEL_FORMAT_YUV_SEMIPLANAR_422) ||
        (drv_layer_ctx->vo_video_attr.pixel_format == OT_PIXEL_FORMAT_YVU_SEMIPLANAR_422)) {
        cvfir_en = TD_FALSE;
    } else {
        cvfir_en = TD_TRUE;
    }
...
td_s32 vo_drv_check_layer_attr_display_rect(ot_vo_layer layer, const ot_rect *rect)
{
    td_u32 max_width;
    td_u32 max_height;

    if ((layer >= VO_SW_LAYER_VHD0) && (layer <= VO_SW_LAYER_VHD0)) {
