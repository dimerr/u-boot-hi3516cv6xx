...
static void ot_reg_set_bit(unsigned long value, unsigned long offset, unsigned long addr)
{
...
{
    const td_u32 tmp = (is_run == TD_TRUE) ? 1 : 0;

    ot_reg_set_bit(tmp, 4, JPEGD_CLK_CRG_REG_ADDR); /* 4: clock gate position */
}
...
void jpegd_reset_select(int vpu_id, td_bool is_run)
{
    ...
    jpgd_regs_type *jpegd_reg = (jpgd_regs_type *)jpegd_get_reg_addr(vpu_id);
    ...
    ot_reg_set_bit(tmp, 0, JPEGD_RST_CRG_REG_ADDR); /* 0: reset position */
    return;
}
...
{
    jpgd_regs_type *jpegd_reg = TD_NULL;

    jpegd_reg = (jpgd_regs_type *)JPEGD_REGS_ADDR;
...
unsigned int jpegd_read_int(int vpu_id)
{
    ...
    return jpegd_reg->reg0_config[0].reg0_intstate.u32;
#else
...
#ifdef MSG_LOAD_ENABLE
    jpegd_reg->reg0_config[0].reg0_intclr.u32 = 0xffffffff; /* 0xffffffff:interrupt value */
#else
...
void jpegd_set_int_mask(int vpu_id)
{
    jpgd_regs_type *jpegd_reg = (jpgd_regs_type *)jpegd_get_reg_addr(vpu_id);
#ifdef MSG_LOAD_ENABLE
    jpegd_reg->reg0_config[0].reg0_intmask.u32 = 0x0; /* 0x0:interrupt mask value */
#else
...
void jpegd_start_vpu(int vpu_id, jpegd_vpu_config *config)
{
    ...
    d32.u32 = 0;
    d32.bits.start_0 = 1;
    jpegd_reg->reg0_config[0].reg0_start.u32 = d32.u32;
#else
...
static void jpegd_resume_start(jpgd_regs_type *jpegd_reg)
{
...
static void jpegd_stride(jpgd_regs_type *jpegd_reg, const jpegd_vpu_config *config)
{
    u_jpeg_stride d32;
...
static void jpegd_picture_size(jpgd_regs_type *jpegd_reg, const jpegd_vpu_config *config)
{
    u_picture_size d32;
...
static void jpegd_picture_type(jpgd_regs_type *jpegd_reg, const jpegd_vpu_config *config)
{
    u_picture_type d32;
...
static void jpegd_rgb_out_stride(jpgd_regs_type *jpegd_reg, const jpegd_vpu_config *config)
{
...
static void jpegd_bit_buffer_addr(jpgd_regs_type *jpegd_reg, const jpegd_vpu_config *config)
{
...
static void jpegd_bit_stream_addr(jpgd_regs_type *jpegd_reg, const jpegd_vpu_config *config)
{
...
static void jpegd_pic_yuv_start_addr(jpgd_regs_type *jpegd_reg, const jpegd_vpu_config *config)
{
...
static void jpegd_freq_scale(jpgd_regs_type *jpegd_reg, const jpegd_vpu_config *config)
{
    u_freq_scale d32;
    d32.u32 = jpegd_reg->freq_scale.u32;
...
static void jpegd_out_type(jpgd_regs_type *jpegd_reg, const jpegd_vpu_config *config)
{
    u_out_type d32;
...
static void jpegd_alpha(jpgd_regs_type *jpegd_reg, const jpegd_vpu_config *config)
{
...
static void jpegd_emar_buffer(jpgd_regs_type *jpegd_reg, const jpegd_vpu_config *config)
{
    unsigned int i;
...
static void jpegd_pic_cut_pos(jpgd_regs_type *jpegd_reg, const jpegd_vpu_config *config)
{
    if (config->out_yuv == TD_FALSE) {
        {
...
static void jpegd_sampling_factor(jpgd_regs_type *jpegd_reg, const jpegd_vpu_config *config)
{
...
static void jpegd_dri(jpgd_regs_type *jpegd_reg, const jpegd_vpu_config *config)
{
    u_dri d32;
...
static void jpegd_config_quant_table_regs(jpgd_regs_type *jpegd_reg, const jpegd_vpu_config *config)
{
    unsigned int i;
...
static void jpegd_config_huffman_table_regs(jpgd_regs_type *jpegd_reg, const jpegd_vpu_config *config)
{
    unsigned int i;
...
#ifdef MSG_LOAD_ENABLE
static void jpegd_write_msg_addr_reg(jpgd_regs_type *reg, const jpegd_vpu_config *config)
{
    reg->reg0_config[0].reg0_addr_l.u32 = get_low_addr(config->phy_emar_buffer0);
    reg->reg0_config[0].reg0_addr_h.u32 = get_high_addr(config->phy_emar_buffer0);
}

static void jpegd_write_priority_reg(jpgd_regs_type *reg)
{
    u_reg0_prio d32;
    d32.u32 = 0;
    d32.bits.prio_lvl_0 = 0;
    reg->reg0_config[0].reg0_prio.u32 = d32.u32;
}
#endif

void jpegd_drv_write_regs(jpgd_regs_type *reg_base, const jpegd_vpu_config *config)
{
    jpgd_regs_type *jpegd_reg = reg_base;

#ifdef MSG_LOAD_ENABLE
    jpegd_write_msg_addr_reg(jpegd_reg, config);
    jpegd_write_priority_reg(jpegd_reg);
#else
    /*  1. jpeg_resume_start */
...
{
    const jpgd_regs_type *jpegd_reg = reg_base;

    if (jpegd_reg == TD_NULL || vpu_status == TD_NULL) {
        printf("jpegd_reg or vpu_status is TD_NULL!\n");
        return;
    }
#ifdef MSG_LOAD_ENABLE
    vpu_status->int_dec_finish = jpegd_reg->reg0_config[0].reg0_intstate.bits.int_dec_fnsh_0;
    vpu_status->int_dec_err = jpegd_reg->reg0_config[0].reg0_intstate.bits.int_dec_err_0;
    vpu_status->int_over_time = jpegd_reg->reg0_config[0].reg0_intstate.bits.int_over_time_0;
    vpu_status->int_bs_res = jpegd_reg->reg0_config[0].reg0_intstate.bits.int_bs_res_0;
#else
    vpu_status->int_dec_finish = jpegd_reg->jpeg_int.bits.int_dec_fnsh;
    vpu_status->int_dec_err = jpegd_reg->jpeg_int.bits.int_dec_err;
    vpu_status->int_over_time = jpegd_reg->jpeg_int.bits.int_over_time;
    vpu_status->int_bs_res = jpegd_reg->jpeg_int.bits.int_bs_res;
#endif
}
