...
__inline static unsigned int align_up(unsigned int x, unsigned int a)
{
    return (x + (a - 1)) & (~(a - 1));
}
...
#define ONE_EMAR_BUF_SIZE     (((JPEGD_MAX_WIDTH + 7) >> 3) << 7)
...
__inline static unsigned int jpegd_min(unsigned int a, unsigned int b)
{
    return (a < b) ? a : b;
}
