...
class AreaTool:
    def __init__(self, alg):
        ...
        if alg != None:
            chk.check_option(alg, [CfgValue.CryptoAlg.RSA3072, CfgValue.CryptoAlg.SM], 'algorithm')
    ...
    def import_pub_key(self, key_file, key_name=None, alg=None):
        ...
        if self._asym_alg == 'RSA3072' or alg == 'RSA3072':
    ...
    def sign(self, data, priv_key_file, data_name=None, alg=None):
        if data_name != None and self._asym_alg != None:
            log.info('[%s] Signing %s' % (self._asym_alg, data_name))

        if self._asym_alg == 'RSA3072' or self._asym_alg == 'RSA' or alg == 'RSA':
    ...
    def enc_cbc(self, data, key, iv, decrypt=False, data_name=None, alg=None):
        ...
        elif  self._enc_cbc_alg == 'AES-128-CBC' or alg == 'AES-128-CBC':
            chk.check_empty(data, notice=data_name)
