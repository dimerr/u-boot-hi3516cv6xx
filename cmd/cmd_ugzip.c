...
static int check_and_get_gzip_info(unsigned long offset, unsigned int *size)
{
	...
	int ret = flash_read(offset, RUGZIP_READ_TMP_SIZE, tmp_addr, NULL);
	if (ret < 0) {
...
{
	int ret = flash_read(offset, gzip_file_size, src_addr, NULL);
	if (ret < 0) {
	...
	unsigned char *src_addr, unsigned char *dst_addr, unsigned int fragmentation_size)
{
	...
	int ret = flash_read(offset, cur_read_size, src_addr, &actual_len);
	if (ret < 0) {
		printf("[error] read gzip file failed(0x%x).\n", ret);
		return CMD_RET_USAGE;
	}

	/* add bad blk size: ret */
	if (actual_len != 0)
		flash_offset += (unsigned int)actual_len;
	else
		flash_offset += cur_read_size;
	...
	unsigned int ddr_offset = cur_read_size;

	while (ddr_offset < gzip_file_size) {

		actual_len = 0;

		if ((ddr_offset + fragmentation_size) < gzip_file_size) {
			cur_read_size = fragmentation_size;
			op_type = HW_DECOMPRESS_OP_CONTINUE;
		} else {
			cur_read_size = gzip_file_size - ddr_offset;
			op_type = HW_DECOMPRESS_OP_END;
		}

		ret = flash_read(offset + flash_offset, cur_read_size, src_addr + ddr_offset, &actual_len);
		if (ret < 0) {
		...
		ret = hw_dec_decompress_ex(op_type,
			dst_addr, &size_uncomparessed, src_addr + ddr_offset, cur_read_size);
		if (ret != 0) {
			printf("[error] [%s] [%d]decompress file failed.\n", __func__, __LINE__);
			hw_dec_uinit();
			return CMD_RET_USAGE;
...
static int do_read_gzip(struct cmd_tbl *cmdtp, int flag, int argc, char * const argv[])
{
	...
	ret = flash_read(offset, gzip_file_size, dst_addr, NULL);
	if (ret < 0) {
...
#define MAX_FRAGMENTATION_SIZE 0x1000000
...
static int do_read_and_ugzip(struct cmd_tbl *cmdtp, int flag, int argc, char * const argv[])
{
...
#if CONFIG_IS_ENABLED(CMD_TIMESTAMP)
	timestamp_mark("rugzip_start", __LINE__);
#endif
	...
	unsigned long long fragmentation_size = simple_strtoull(argv[4], NULL, 16);
	...
	if (fragmentation_size > MAX_FRAGMENTATION_SIZE) {
		printf("[error] fragmentation_size larger than 0x%x!\n", MAX_FRAGMENTATION_SIZE);
		return CMD_RET_USAGE;
	}
	...
	dcache_enable(); /* enable dcache */
#if CONFIG_IS_ENABLED(CMD_TIMESTAMP)
	timestamp_mark("rugzip_end", __LINE__);
#endif
