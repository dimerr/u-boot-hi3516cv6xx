...
static int do_vobg(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
    int ret;
...
static int do_startgx(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
    int ret;
...
static int do_stopgx(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
    int ret;
    unsigned int layer;
...
{
    return stopgx_parse(argv, layer);
}
...
#if (defined(CHIP_HI3519DV500))

#define USAGE_DEV_STRING                          \
	"\t-<dev> : 0(DHD0)\n"

#define USAGE_INTF_STRING                                                                  \
	"\t-<intftype>: 4(BT.656), 8(BT.1120), 32(RGB_6BIT), 64(RGB_8BIT)\n"                   \
