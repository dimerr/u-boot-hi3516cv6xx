	...
	if ((s32ret == 0) && pstitem) {
		...
		if (ncompressed) {
			...
			if (s32ret == 0 && s32sizecompressed == pstitem->u32itemoriginlen) {
				...
				(void)memset_s((void *)(uintptr_t)uldst, BYTE_ALIGN, 0, BYTE_ALIGN);
			...
			if (memcpy_s((void *)(uintptr_t)uldst, BYTE_ALIGN, (void *)(uintptr_t)ulitemdata,
					pstitem->u32itemlen)) {
...
static int do_cycle(struct cmd_tbl *cmdtp, int flag, int argc, char* const argv[])
{
	...
	ulsrc     = simple_strtoul(argv[1], NULL, BYTE_ALIGN);
	ulsrcbak  = simple_strtoul(argv[2], NULL, BYTE_ALIGN);
	u32srclen = simple_strtoul(argv[3], NULL, BYTE_ALIGN);
	uldst     = simple_strtoul(argv[4], NULL, BYTE_ALIGN);
	...
	if (uldst < PHYS_SDRAM_1 || uldst + u32srclen >= CONFIG_SYS_TEXT_BASE_ORI) {
		printf("ERR:\n  dst[0X%08lx] addr is illegal!\n", uldst);
		return 1;
	}
